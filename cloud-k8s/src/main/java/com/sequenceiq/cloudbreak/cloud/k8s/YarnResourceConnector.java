...
public class YarnResourceConnector implements ResourceConnector<Object> {
    ...
    private CreateApplicationRequest createRequest(CloudStack stack, String applicationName) {
        ...
        createApplicationRequest.setQueue(stack.getParameters().getOrDefault(K8sConstants.YARN_QUEUE_PARAMETER, defaultQueue));
        String lifeTimeStr = stack.getParameters().get(K8sConstants.K8S_LIFETIME_PARAMETER);
        ...
        for (CloudResource resource : resources) {
            switch (resource.getType()) {
                case K8S_APPLICATION:
                    LOGGER.info("Checking K8S application status of: {}", resource.getName());
                    try {
                        ApplicationDetailRequest applicationDetailRequest = new ApplicationDetailRequest();
            ...
            switch (resource.getType()) {
                case K8S_APPLICATION:
                    ...
                    try {
                        ...
                        LOGGER.info("K8S Applicatin has been deleted");
